...
class ARFollower():
    ...
    def __init__(self, tag_id ,x_goal, y_goal, time_goal):
        ...
        self.max_linear_speed = rospy.get_param("~max_linear_speed", 0.15) #0.15   (0.15) 0.085
        
        # The minimum linear speed in meters per second
        self.min_linear_speed = rospy.get_param("~min_linear_speed", 0.057) #0.057  (0.057)
    ...
    def set_cmd_vel(self, msg):
        ...
        if abs(target_offset_x - self.goal_x) > self.x_threshold:
            ...
            if speed < 0:
                speed *= 1.5
            # speed = speed * -1 # If Kinect is on the back
...
class GoToPose():
    ...
    def goto(self, x_goal, y_goal, theta_goal):

        rospy.loginfo("Going to x:%f, " % x_goal + "y:%f, " % y_goal + "theta:%f, " % theta_goal)
...
class Roller ():
    def __init__(self, roller_id, roller_command):
        ...
        self.roller10_cmd_pub = rospy.Publisher('agc03_sensor_omron_cmd', String, queue_size=1)
        # ##################################################################################################
        ...
        self.roller11_cmd_pub = rospy.Publisher('agc03_sensor_omron_cmd', String, queue_size=1)
        # ##################################################################################################
        ...
        while not rospy.is_shutdown():
            ...
            if ((roller_id == 10) and (roller_command == cmd_load)):
                ...
                self.roller10_cmd_pub.publish("on11")
                self.roller10_cmd_pub.publish("on11")
                #rospy.sleep(2)
                #self.roller10_cmd_pub.publish("off1")
                #self.roller10_cmd_pub.publish("off1")
                return
                ...
                self.roller11_cmd_pub.publish("on11")
                self.roller11_cmd_pub.publish("on11")
                #rospy.sleep(2)
                #self.roller11_cmd_pub.publish("off1")
                #self.roller11_cmd_pub.publish("off1")
                rospy.sleep(1)
                return
...
def talker(message):
    '''test Publisher'''
    pub = rospy.Publisher('agc03_status', String, queue_size=1)
...
if __name__ == '__main__':
    ...
    try:
        rospy.init_node('moving_around_2', anonymous=False)
        agc = GoToPose()
        
        while not rospy.is_shutdown():

          success = agc.goto(4.0,-1.0,90.0)
          print success
          if not success:
              success = agc.goto(4.0,-1.0,90.0)
          else:
          ...
          success = agc.goto(2.0,-2.0,-90.0)
          print success
          success = agc.goto(2.0,-7.0,0.0)
          print success
          success = agc.goto(4.0,-5.0,90.0)
          print success
            
          #ARFollower(0, 2.0, 0.007, 10.0) # (tag_id ,x_goal, y_goal, time_goal)

        #ARFollower(18, 0.30, 0.007, 0.0) # (tag_id ,x_goal, y_goal, time_goal)

        #ARFollower(18, 1.8, 0.007, 0.0) # (tag_id ,x_goal, y_goal, time_goal)

          #GoToPose(4.0,-3.0,90.0)    #GoToPose Roller AR 11
#############################################################################################

        #rospy.spin()
    except rospy.ROSInterruptException:
        rospy.loginfo("Cart Tracking is done.")
