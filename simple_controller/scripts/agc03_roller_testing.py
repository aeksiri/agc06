...
def agc03_load_in(roller_id):
    ...
    while not ( rospy.is_shutdown() or flag ):
        rospy.loginfo("AGC03 roller status : %s " % status_of_agc04_roller)
        rospy.loginfo("Roller10 status : %s " % status_of_roller13)
...
def agc03_load_out(roller_id):
    ...
    while not ( rospy.is_shutdown() or flag ):
        rospy.loginfo("AGC03 roller status : %s " % status_of_agc04_roller)
        rospy.loginfo("Roller11 status : %s " % status_of_roller14)
...
if __name__ == '__main__':
    ...
    try:
        rospy.init_node('agc03_roller_testing', anonymous=False)
