...
class ARFollower():
    def __init__(self):
        ...
        self.min_angular_speed = rospy.get_param("~min_angular_speed", 0.15)
        
        # The maximum distance a target can be from the robot for us to track
        self.max_x = rospy.get_param("~max_x", 4.0)
        
        # The goal distance (in meters) to keep between the robot and the marker  ----------------X
        #self.goal_x = rospy.get_param("~goal_x", 0.6)
        self.goal_x = rospy.get_param("~goal_x", 0.9)

        # How far away from the goal distance (in meters) before the robot reacts
        self.x_threshold = rospy.get_param("~x_threshold", 0.04)
        
        # How far away from being centered (y displacement) on the AR marker  --------------------Y
        # before the robot reacts (units are meters)
        self.y_threshold = rospy.get_param("~y_threshold", 0.55)    #0.05
