        ...
        while not rospy.is_shutdown():
           # while self.marker.pose.pose.position.x > x_goal:
            ...
            if (self.data_target_offset_x == 0) and (self.data_target_offset_y == 0): 
                if count_timeout > 25:    # // 50    1/self.rate x timeout = T
...
class AGC04_cmd_status():   
    def __init__(self, AGC04_command, AGC04_status):
        ...
        #received_agc04_cmd
        ###################### Subscribe from Roller15 ###################################################
        rospy.Subscriber('agc04_cmd', String, self.received_agc04_cmd)
        ###################### Publish to Roller15 #######################################################
        #self.agc04_status_pub = rospy.Publisher('agc04_status', String, queue_size=1)
        
        
        #agc04_status_pub.publish(AGC04_status_XX)
        ##################################################################################################

        # Begin the cmd_vel publishing loop
        while not rospy.is_shutdown():
            rospy.sleep(1)
            rospy.loginfo("status_of_received_agc04_cmd : %s " % status_of_received_agc04_cmd)
            if (AGC04_command == status_of_received_agc04_cmd):
                rospy.sleep(1)
                return
            else:
                #rospy.loginfo(".")
                AGC04_status_XX = AGC04_status
                #self.agc04_status_pub.publish(AGC04_status_XX)
                talker(AGC04_status_XX)
                rospy.sleep(1)
                
            r.sleep()


    def received_agc04_cmd(self, msg):
        self.received_agc04_cmd = msg.data
        global status_of_received_agc04_cmd
        status_of_received_agc04_cmd = self.received_agc04_cmd  
        #rospy.loginfo("status_of_received_agc04_cmd : %s " % status_of_received_agc04_cmd) 

    def shutdown(self):
        #rospy.loginfo("Rollers 10,11,12 are stoped")
        #self.roller13_cmd_pub.publish(cmd_stop)
        rospy.loginfo("AGC04_cmd_status stop")
        rospy.sleep(1)
...
if __name__ == '__main__':
    ...
    try:
        ...
        ###########################################################################
            ...
            # agc04_buzzer_cmd_pub.publish("ch1")
            # rospy.sleep(5)

            # agc04_buzzer_cmd_pub.publish("ch2")
            # rospy.sleep(5)

            # agc04_buzzer_cmd_pub.publish("ch3")
            # rospy.sleep(5)

            # agc04_buzzer_cmd_pub.publish("ch4")
            # rospy.sleep(5)
            
            # agc04_buzzer_cmd_pub.publish("ch0")
            # rospy.sleep(5)
        ...
        # while not rospy.is_shutdown():
            ...
            #AGC04_cmd_status("give_me_a_way", "wait_a_minute") 
            # [clear_to_go / wait_a_minute] ["_I_am_gone", "give_me_a_way", "agc02_is_moving"]
            #rospy.loginfo("Roller13 status :------------------------- ")
            #rospy.sleep(1)

        while not rospy.is_shutdown():
            ARFollower(10, 1.5, 0.050)
            #rospy.sleep(3)
            #ARFollower(10, 0.75, 0.010)
            #rospy.sleep(3)
            ARFollower(10, 0.50, 0.005)
            #rospy.sleep(3)
        ...
        #     #AGC04_COMMAND("wait_a_minute")
        #     #AGC04_COMMAND("clear_to_go")
        # #    talker("wait_a_minute")
        #     AGC04_cmd_status("I_am_gone", "clear_to_go")
        #     clear_costmaps(10)
        #     rospy.sleep(1)
        #     talker("wait_a_minute")
        #     rospy.sleep(1)
        ...
        #     rospy.loginfo("status :----------goto Roller AR 14----------- ")

        #     #voice1_on_pub.publish()
        #     ARFollower(13, 1.2, 0.05)
        #     rospy.sleep(1)
        ...
        #     rospy.loginfo("status :----------goto Roller AR 13----------- ")
