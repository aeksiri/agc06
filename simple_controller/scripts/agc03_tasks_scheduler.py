        ...
        while not rospy.is_shutdown():
           # while self.marker.pose.pose.position.x > x_goal:
            ...
            if (self.data_target_offset_x == 0) and (self.data_target_offset_y == 0): 
                if count_timeout > self.time_out :    # 1/self.rate x timeout = T
...
class Roller ():
    def __init__(self, roller_id, roller_command):
        ...
        self.cmd_load = "load"
        self.cmd_unload = "unload"
        self.cmd_stop = "stop"
        self.cmd_mload = "mload"
        self.cmd_munload = "munload"
        ...
        while not rospy.is_shutdown():
            ...
            if ((roller_id == 10) and (roller_command == self.cmd_load)):
                self.roller10_cmd_pub.publish(self.cmd_load)
                rospy.sleep(1)
                return
            elif ((roller_id == 10) and (roller_command == self.cmd_unload)):
                self.roller10_cmd_pub.publish(self.cmd_unload)
                rospy.sleep(1)
                return
            elif ((roller_id == 11) and (roller_command == self.cmd_load)):
                self.roller11_cmd_pub.publish(self.cmd_load)
                rospy.sleep(1)
                return
            elif ((roller_id == 11) and (roller_command == self.cmd_unload)):
                self.roller11_cmd_pub.publish(self.cmd_unload)
                rospy.sleep(1)
                return
            elif ((roller_id == 12) and (roller_command == self.cmd_load)):
                self.roller12_cmd_pub.publish(self.cmd_load)
                rospy.sleep(1)
                return
            elif ((roller_id == 12) and (roller_command == self.cmd_unload)):
                self.roller15_cmd_pub.publish(self.cmd_unload)
                rospy.sleep(1)
                return
    ...
    def shutdown(self):
        self.roller10_cmd_pub.publish(self.cmd_stop)
        self.roller11_cmd_pub.publish(self.cmd_stop)
        self.roller12_cmd_pub.publish(self.cmd_stop)
...
class AGC03_COMMAND() :
    def __init__(self, status):
        ...
        global sending_agc03_status
        sending_agc03_status = status

        # Set the shutdown function (stop the agc03 command)
        rospy.on_shutdown(self.shutdown)
        ...
        rospy.Subscriber('agc03_cmd', String, self.recieved_cmd)
        rospy.sleep(0.4)
        ######################### Publish to AGC02 #######################################################
        self.roller10_cmd_pub = rospy.Publisher('agc03_status', String, queue_size=1)
        ##################################################################################################

        # Begin the agc03_status publishing loop
        while not rospy.is_shutdown():
            self.roller10_cmd_pub.publish(sending_agc03_status)
            self.r.sleep()
            return

    def recieved_cmd(self, msg):
        #global received_agc03_cmd
        temp_cmd = msg.data
        self.r.sleep()
        return temp_cmd

    def shutdown(self):
        rospy.loginfo("AGC03_CMD stopped")
        rospy.sleep(1)

def agc03_load_in(roller_id):
    AGC03Roller("load")
    rospy.sleep(1)
    ...
    while not ( rospy.is_shutdown() or flag ):
        ...
        #if (status_of_roller10 == "roller10_complete_unload") and (status_of_agc03_roller == "complete_load"):
...
if __name__ == '__main__':
    try:
        ...
        while not rospy.is_shutdown():
            ...
            rospy.sleep(0.5)
            ARFollower(10, 1.2, 0.05, 1)
            rospy.sleep(0.5)
            agc03_buzzer_cmd_pub.publish("ch2")
            rospy.sleep(0.5)

            GoToPose(0.93,3.2,-90)
            agc03_buzzer_cmd_pub.publish("ch3")
            rospy.sleep(0.5)

            ARFollower(11, 0.5, 0.006, 16)
            rospy.sleep(0.5)
            agc03_buzzer_cmd_pub.publish("ch0")
            rospy.sleep(0.5)
            ...
            GoToPose(1.40, 2.5, 90)
            rospy.sleep(0.5)

            ARFollower(10, 0.53, 0.006, 16)
            rospy.sleep(0.5)
